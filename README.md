...
`DeviceControl` carries the 2022 image-snapshot command (`SnapShot`: `SnapNum`/`Interval`/`UploadURL`/`SessionID` per A.2.1.24), `manscdp.Decode` parses the `UploadSnapShotFinished` completion notify (A.2.5.7, `SnapShotList` of `SnapShotFileID`), and `platform.PTZController` grows `SendSnapShotCmd` / `StartManualRecord` / `StopManualRecord`. A playback/download session that completes naturally now sends the in-dialog §9.4.2 `MediaStatus: Play Finished` / `Download Finished` SIP INFO (MANSRTSP-style body, byte-compatible with the Rust twin) so platforms stop the fetch immediately; BYE-side teardown stays silent. The §9.3.2 management controls land end-to-end (issue #81): `DeviceControl` decodes `IFrameCmd` (force IDR — platforms send it when starting a pull or after loss), `RecordCmd`, `GuardCmd`, `AlarmCmd`, `TeleBoot` and raw `PTZCmd` passthrough, `device.SetControlHandlers` fires optional per-command host callbacks (installed-callback-only execution; anything else keeps the explicit control reject), and `platform.PTZController` grows `SendIFrameCmd`. Wire values mirror gb28181-rs #58; cross-twin goldens pin them. Subscriptions (issue #80) landed end-to-end too: platform SUBSCRIBEs (Catalog / Alarm / MobilePosition) are booked with renewal/expiry and answered 200 OK echoing `Expires`; `device.Server.Notifier()` hands the host `SendAlarm` / `SendCatalogChange` / `SendMobilePosition` (no-ops until subscribed) and `SetPositionSource` drives periodic position reports on the SUBSCRIBE `Interval` — NOTIFY wire shapes mirror the Rust twin (gb28181-rs #66). Voice talkback receive (§9.2) completes the 2022 story: an audio-only INVITE offer (`m=audio`, no `m=video`) opens a talkback session — the device answers with an ephemeral UDP port, strips the RTP header, and feeds G.711 A/μ-law payload to the host sink installed via `device.SetTalkbackSink` (`OnAudio(payload, ssrc, codec)`); no sink, non-G.711 codec, or TCP media is refused with 488. Ported from the production-verified gb28181-rs reference — goldens pin the byte-exact answer and the real NVR offer form (payload-type-only PCMA without `a=rtpmap`, leading-zero decimal `y=` SSRC, no trailing CRLF). PTZCmd now arrives bit-level decoded (GB/T 28181 §A.3-A.4): `DecodePTZCommand` inverts the platform-side builders — movement direction bits + per-axis speeds, preset set/call/delete, cruise instructions, FI focus/iris, auxiliary switches — with byte-exact goldens shared with the Rust twin; `OnPTZCmd` receives the structured command (undecodable hex is preserved as `Kind=PtzInvalid`). SIP-Date time sync observation (2022 §9.10.2): the REGISTER response's `Date` header is parsed (RFC 3261 §25.1 forms) and exposed via `Server.PlatformDateUnix()`; drift beyond 5s is logged — applying the clock stays with the host. DeviceConfig lands with the same stance (§9.3.3 / A.2.3.2, wire forms verified against the 2022 standard text): `SetConfigHandlers` decodes the subset a fixed camera acts on — `BasicParam` (name + registration tuning), `FrameMirror` (0-3 flip modes), `AlarmReport` (motion/field report switches) — a callback-executed command answers `Result=OK`, the rest keep the reject; ConfigDownload queries (A.2.4.7) answer the minimal valid Response with the `BasicParam` block from the live config when requested. `HomePosition` joins the DeviceControl callbacks (A.2.3.1.10) and `platform.PTZController` gains a struct-form `SetHomePosition`. Note: 2022 校时 is NOT a DeviceConfig sub-command — §9.10.2 does it via the REGISTER response's SIP Date header.
