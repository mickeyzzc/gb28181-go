...
// invite is the request already handed to the transaction layer; building
// ACKs from it after gosip owns it is safe because gosip (>= 798b72c)
// clones instead of mutating the shared Via in SetViaHop (#95/#87).
func (s *Server) awaitInviteAnswer(srv gosip.Server, tx sip.ClientTransaction, invite sip.Request) (sip.Response, error) {
	...
	for {
		...
		case <-speculative.C:
			speculative.Stop()
			if err := srv.Send(buildSpeculativeAck(invite)); err != nil {
