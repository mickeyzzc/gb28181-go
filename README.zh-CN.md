...
`DeviceControl` 携带 2022 版图像抓拍命令（`SnapShot`：`SnapNum`/`Interval`/`UploadURL`/`SessionID`，A.2.1.24）；`manscdp.Decode` 解析 `UploadSnapShotFinished` 完成通知（A.2.5.7，`SnapShotList`/`SnapShotFileID`）；`platform.PTZController` 新增 `SendSnapShotCmd` / `StartManualRecord` / `StopManualRecord`。 回放/下载会话自然结束时发送会话内 §9.4.2 `MediaStatus: Play Finished` / `Download Finished` SIP INFO（MANSRTSP 风格消息体，与 Rust 孪生字节一致），平台可立即停止拉流；BYE 侧拆除保持静默。UDP 传输。 §9.3.2 管理控制命令端到端落地（issue #81）：`DeviceControl` 解码 `IFrameCmd`（强制 I 帧——平台起流/丢包恢复时常发）、`RecordCmd`、`GuardCmd`、`AlarmCmd`、`TeleBoot` 与 `PTZCmd` 原样透传；`device.SetControlHandlers` 按命令触发可选宿主回调（仅安装了回调才执行，其余保持显式 control reject）；`platform.PTZController` 新增 `SendIFrameCmd`。线格式取值与 gb28181-rs #58 一致，跨孪生 golden 钉板。 订阅（issue #80）也已端到端落地：平台 SUBSCRIBE（Catalog / Alarm / MobilePosition）按事件簿记（续订/过期）并回 200 OK 回显 `Expires`；`device.Server.Notifier()` 向宿主提供 `SendAlarm` / `SendCatalogChange` / `SendMobilePosition`（未订阅时安全 no-op），`SetPositionSource` 按订阅 `Interval` 周期上报位置——NOTIFY 线格式对齐 Rust 孪生（gb28181-rs #66）。 语音对讲接收半边（§9.2）补齐 2022 能力：纯音频 INVITE offer（`m=audio`、无 `m=video`）即对讲会话——设备以临时 UDP 端口应答，剥离 RTP 头后把 G.711 A/μ 律载荷交给 `device.SetTalkbackSink` 安装的宿主 sink（`OnAudio(payload, ssrc, codec)`）；无 sink、非 G.711 编码或 TCP 媒体一律 488 拒绝。移植自生产验证过的 gb28181-rs 参考实现——golden 钉板字节级应答与真实 NVR offer 形态（仅 payload-type 的 PCMA 无 `a=rtpmap`、前导零十进制 `y=` SSRC、无尾部 CRLF）。 PTZCmd 现以位级解码送达（GB/T 28181 §A.3-A.4）：`DecodePTZCommand` 为 platform 侧构造器的逆运算——运动方向位+各轴速度、预置位设置/调用/删除、巡航指令、FI 聚焦/光圈、辅助开关——golden 与 Rust 孪生共享同一字节表；`OnPTZCmd` 接收结构化命令（无法解码的十六进制以 `Kind=PtzInvalid` 原样保留）。 SIP-Date 校时观测（2022 §9.10.2）：解析 REGISTER 应答的 `Date` 头并经 `Server.PlatformDateUnix()` 暴露；漂移超 5 秒记日志——是否校时由宿主决定。 DeviceConfig 以同样姿态落地（§9.3.3 / A.2.3.2，线格式经 2022 标准原文核验）：`SetConfigHandlers` 解码固定相机可执行的子集——`BasicParam`（名称+注册参数）、`FrameMirror`（0-3 翻转模式）、`AlarmReport`（移动侦测/区域入侵上报开关）——回调执行的命令答 `Result=OK`，其余保持 reject；ConfigDownload 查询（A.2.4.7）按请求回最小合规 Response（请求含 BasicParam 时带上运行配置的对应块）。`HomePosition` 并入 DeviceControl 回调（A.2.3.1.10），`platform.PTZController` 新增结构化 `SetHomePosition`。注意：2022 校时不是 DeviceConfig 子命令——§9.10.2 走 REGISTER 应答的 SIP Date 头。
