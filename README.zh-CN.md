...
`DeviceControl` 携带 2022 版图像抓拍命令（`SnapShot`：`SnapNum`/`Interval`/`UploadURL`/`SessionID`，A.2.1.24）；`manscdp.Decode` 解析 `UploadSnapShotFinished` 完成通知（A.2.5.7，`SnapShotList`/`SnapShotFileID`）；`platform.PTZController` 新增 `SendSnapShotCmd` / `StartManualRecord` / `StopManualRecord`。 回放/下载会话自然结束时发送会话内 §9.4.2 `MediaStatus: Play Finished` / `Download Finished` SIP INFO（MANSRTSP 风格消息体，与 Rust 孪生字节一致），平台可立即停止拉流；BYE 侧拆除保持静默。UDP 传输。 §9.3.2 管理控制命令端到端落地（issue #81）：`DeviceControl` 解码 `IFrameCmd`（强制 I 帧——平台起流/丢包恢复时常发）、`RecordCmd`、`GuardCmd`、`AlarmCmd`、`TeleBoot` 与 `PTZCmd` 原样透传；`device.SetControlHandlers` 按命令触发可选宿主回调（仅安装了回调才执行，其余保持显式 control reject）；`platform.PTZController` 新增 `SendIFrameCmd`。线格式取值与 gb28181-rs #58 一致，跨孪生 golden 钉板。
